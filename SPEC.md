# Tacticool KML v1.4.2 Specification
...
- Initially collapsed
- Starts at Bullseye
- Inputs: radial, length in NM, name, and color
- Default name is `AXIS {radial/reciprocal} {B/E coordinate}`
...
- Tickmark Group details show the line count, Main/Sub settings, and distance label for each tickmark line
...
- Inputs: full circle toggle, range ring interval, start radial, end radial, Major radial interval, Minor radial interval, start range, end range, spider color, and Cardinal color
...
- Major radial interval defaults to 30 degrees and must be an integer from 1 to 180 degrees
- Minor radial interval defaults to 10 degrees; blank means no Minor radials are generated
...
- Minor radials overlapping Major or Cardinal radials are skipped
...
- Optional internal lines can be generated by:
  - Width divisions: equal divisions along the Width axis
  - Depth divisions: equal divisions along the Depth axis
  - Width ratio: unequal ratio divisions along the Width axis, such as `7:3`
  - Depth ratio: unequal ratio divisions along the Depth axis, such as `7:3`
- A single axis cannot use both equal divisions and ratio divisions at the same time
...
- Preview coastline opacity is fixed at high visibility
