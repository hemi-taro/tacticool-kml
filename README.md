...
1. Magnetic VariationをAuto、Manual、またはNone (True HDG)から選択します。
2. Bullseye座標を入力します。
...
SAM Ringは`At B/E`を選択でき、名前を省略すると`SAM {中心座標}`になります。BoxとArcは生成時にNameが空なら`BOX {中心座標}`または`ARC {中心座標}`を自動入力します。Axisの既定名は`AXIS {方位/反方位} {B/E座標}`です。
...
距離単位はNMです。Auto/Manualでは入力方位を磁方位として扱い、東偏を正、西偏を負として真方位へ変換します。None (True HDG)では入力方位を真方位として扱い、磁気偏差補正を行いません。Auto magnetic variationはB/EまたはDirect Box/Arcの中心座標、現在日付、海面高度からWMM2025で計算します。
