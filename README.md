...
`Share KML`は共有シートを開きます。ForeFlightが共有候補に表示されない場合は、`Save KML to Files`で保存し、ファイルアプリからForeFlightへ共有してください。更新後に古い画面が残る場合は、SafariでGitHub Pagesを一度開いて再読み込みしてからPWAを起動してください。
